...
HakureiTerminal stores application settings, non-executable client-authored character drafts, user roles, disposable remote display caches, legacy inert conversations/runtime records, appearance settings, media, external connection definitions, and diagnostic logs on the device. GensokyoAI `2026.7.14.0`, not this local data, is the sole authority for executable characters, sessions, messages, context, memory, scenes, tools, timers, and generation state.
...
HakureiTerminal has no first-party chat runtime and never calls a model Provider directly. Model profiles are retained only so the user can explicitly delegate selected settings to a selected GensokyoAI Runtime; the Runtime and Provider operators control their own processing and retention.

For an enabled external GensokyoAI connection, HakureiTerminal may send:
...
Client-authored character drafts are local, non-executable content. Editing, importing, restoring, or opening the application never uploads them. Character upload is currently unavailable. Any future upload requires a documented public GensokyoAI network API plus a separate user action that confirms the destination connection and payload scope.

Saving a connection and opening the application do not make a request. Testing, enabling, connecting, and using an external session can make network requests only after an explicit user action. Importing a `.jovarchive` does not contact a Runtime or Provider; imported external connections are disabled and Provider delegation is cleared.
...
- Import restores credentials but leaves external connections disabled and clears Provider delegation until the user explicitly configures them again.
